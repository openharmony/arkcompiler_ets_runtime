...
{
    [[maybe_unused]] PostTaskResult res = CheckAndPostTask(TerminateDaemonTask(nullptr));
    ASSERT(res == PostTaskResult::SUCCESS);
    thread_->join();
    thread_.reset();
...
DaemonThread::PostTaskResult DaemonThread::CheckAndPostTask(DaemonTask task)
{
    LockHolder holder(mtx_);
    if (UNLIKELY(HasRequestedTermination())) {
        LOG_GC(ERROR) << "Try to post task to daemon, but daemon thread is not running.";
        return PostTaskResult::DAEMON_THREAD_NOT_RUNNING;
    }
    ...
    return PostTaskResult::SAME_GROUP_TASK_ALREADY_POSTED;
}
