...
void ConstantPoolProcessor::CollectConstantPoolInfo(const JSPandaFile* pf, JSHandle<JSTaggedValue> constantPool)
{
    ...
    auto value = GenerateConstantPoolInfo(pf, cp);
    array->Set(thread, index_++, value);
}

JSTaggedValue ConstantPoolProcessor::GenerateConstantPoolInfo(const JSPandaFile* pf, JSHandle<ConstantPool> constPool)
{
    JSThread *thread = vm_->GetJSThread();
    ObjectFactory *factory = vm_->GetFactory();
    TSManager *tsManager = vm_->GetTSManager();
    const panda_file::File *pfile = pf->GetPandaFile();
    panda_file::File::IndexHeader *indexHeader = constPool->GetIndexHeader();

    auto stringIndexTable = tsManager->getStringIndexTable();
    Span<const panda_file::File::EntityId> indexs = pfile->GetMethodIndex(indexHeader);

    uint32_t len = stringIndexTable.size();
    JSHandle<TaggedArray> valueArray = factory->NewTaggedArray(len * ITEM_SIZE);

    int index = 0;
    for (uint32_t constantPoolIndex : stringIndexTable) {
        panda_file::File::EntityId id = indexs[constantPoolIndex];
        ...
        valueArray->Set(thread, index++, JSTaggedValue(constantPoolIndex));
        valueArray->Set(thread, index++, JSTaggedValue(string));
    }
