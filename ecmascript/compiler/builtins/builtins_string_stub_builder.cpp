    ...
    GateRef sourceLength, GateRef size, VariableType type)
{
    ...
    LoopBegin(&loopHead);
    {
        ...
        Label body(env);
        BRANCH(Int32GreaterThanOrEqual(*len, Int32(elemInBatch)), &body, &storeTail); // len>=16
        Bind(&body);
        {
    ...
    Bind(&storeTail);
    {
        ...
        // 16 > len >= 8
        BRANCH(Int32GreaterThanOrEqual(*len, Int32(elemInInt64)), &storeTail8_16, &storeTail0_8);
        Bind(&storeTail8_16);
        {
            GateRef elem = Load(VariableType::INT64(), *sourceTmp);
            Store(VariableType::INT64(), glue, *dstTmp, IntPtr(0), elem);
        ...
        Bind(&storeTail0_8);
        {
            ...
            LoopBegin(&tailLoopHead);
            {
                Label body(env);
                // 8 > len > 0
                BRANCH(Int32GreaterThan(*len, Int32(0)), &body, &exit);
                Bind(&body);
                {
