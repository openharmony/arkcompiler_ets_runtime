...
void BuiltinsObjectStubBuilder::ToStringFunc(Variable *result, Label *exit, Label *slowPath)
{
    ...
    Bind(&ecmaObj);
    {
        ...
        GateRef toStringTagSymbol = GetGlobalEnvValue(VariableType::JS_ANY(), glue_, glueGlobalEnv,
    ...
    GateRef toAssign, GateRef source)
{
    ...
    BRANCH(IsDictionaryMode(glue_, elements), &dictionaryMode, &notDictionaryMode);
    ...
    GateRef toAssign, GateRef source)
{
    ...
    Label dictionaryMode(env);
    Label notDictionaryMode(env);
    BRANCH(IsDictionaryMode(glue_, properties), &dictionaryMode, &notDictionaryMode);
    Bind(&notDictionaryMode);
    {
        ...
        Bind(&keyIsString);
        {
            ...
            Bind(&isIndex);
            {
                ...
                Bind(&findByIndex);
                {
                    ...
                    BRANCH(IsDictionaryMode(glue_, elements), &isDictionaryElement, &notDictionaryElement);
...
GateRef BuiltinsObjectStubBuilder::CopyFromEnumCache(GateRef glue, GateRef elements)
{
    ...
    LoopBegin(&loopHead);
    {
        ...
        Bind(&storeValue);
        {
            GateRef value = GetValueFromTaggedArray(glue, elements, Int32Add(*index,
...
GateRef BuiltinsObjectStubBuilder::GetAllEnumKeys(GateRef glue, GateRef obj)
{
    ...
    BRANCH(IsDictionaryMode(glue, array), &isDictionary, &notDictionary);
    Bind(&isDictionary);
    {
...
void BuiltinsObjectStubBuilder::GetPrototypeOf(Variable *result, Label *exit, Label *slowPath)
{
    ...
    Bind(&noPendingException);
    {
        ...
        Bind(&isEcmaObject);
        {
            BRANCH(IsJsProxy(glue_, obj), slowPath, &notJsProxy);
            Bind(&notJsProxy);
            {
...
void BuiltinsObjectStubBuilder::SetPrototypeOf(Variable *result, Label *exit, Label *slowPath)
{
    ...
    Bind(&checkJsObj);
    {
        ...
        Bind(&checkProto);
        {
            ...
            BRANCH(IsEcmaObject(glue_, obj), &isEcmaObject, &notEcmaObject);
            Bind(&isEcmaObject);
            Jump(&setProto);
            Bind(&notEcmaObject);
            {
...
GateRef BuiltinsObjectStubBuilder::ObjectSetPrototype(GateRef glue, GateRef obj, GateRef proto)
{
    ...
    Bind(&notEqual);
    {
        BRANCH(IsExtensible(glue, obj), &isExtensible, &notExtensible);
        Bind(&isExtensible);
        {
...
GateRef BuiltinsObjectStubBuilder::GetAllPropertyKeys(GateRef glue, GateRef obj, GateRef offset, GateRef array)
{
    ...
    BRANCH(IsDictionaryMode(glue, properties), &isDictionary, &notDictionary);
    Bind(&isDictionary);
    {
        FatalPrint(glue, { Int32(GET_MESSAGE_STRING_ID(ThisBranchIsUnreachable)) });
        Jump(&exit);
...
GateRef BuiltinsObjectStubBuilder::GetEnumElementEntries(GateRef glue, GateRef obj, Label* slowPath)
{
    ...
    BRANCH(TaggedIsString(glue, els), slowPath, &notPrimitiveString);
    Bind(&notPrimitiveString);
    BRANCH(IsDictionaryMode(glue, elements), slowPath, &notDictMode);
    Bind(&notDictMode);
    {
        Label loopHead(env);
        Label loopEnd(env);
...
GateRef BuiltinsObjectStubBuilder::GetEnumPropertyEntries(GateRef glue, GateRef obj, Label* slowPath)
{
    ...
    Label notDictionary(env);
    GateRef array = GetPropertiesArray(glue, obj);
    BRANCH(IsDictionaryMode(glue, array), slowPath, &notDictionary);
    Bind(&notDictionary);
...
void BuiltinsObjectStubBuilder::IsFrozen(Variable *result, Label *exit, Label *slowPath)
{
    ...
    BRANCH(BoolNot(IsEcmaObject(glue_, obj)), &notEcmaObj, &isEcmaObj);
    Bind(&notEcmaObj);
    {
...
void BuiltinsObjectStubBuilder::GetOwnPropertyDescriptors(Variable *result, Label *exit, Label *slowPath)
{
    ...
    BRANCH(IsDictionaryMode(glue_, properties), slowPath, &notDictMode);
    Bind(&notDictMode);
    ...
    LoopBegin(&loopHead);
    {
        ...
        Bind(&next);
        {
            ...
            Bind(&isAccessor);
            {
                ...
                BRANCH_UNLIKELY(TaggedIsInternalAccessor(glue_, *value), &isInternalAccesstor, &notInternalAccesstor);
