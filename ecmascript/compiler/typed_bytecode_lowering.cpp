...
void TypedBytecodeLowering::LoadOnPrototypeForHeapObjectReceiver(const LoadObjByNameTypeInfoAccessor &tacc,
    Variable &result, LoadObjByNameOnProtoTypeInfo ldProtoInfo)
{
    ...
    if (!protoConstantFound) {
        ...
        auto curHC = builder_.LoadConstOffset(
            VariableType::JS_POINTER(), *current, TaggedObject::HCLASS_OFFSET);
...
void TypedBytecodeLowering::LowerTypedMonoLdObjByNameOnProto(const LoadObjByNameTypeInfoAccessor &tacc,
                                                             Variable &result)
{
...
void TypedBytecodeLowering::LowerTypedMonoLdObjByName(const LoadObjByNameTypeInfoAccessor &tacc)
{
    GateRef receiver = tacc.GetReceiver();
    GateRef gate = tacc.GetGate();
    ...
    const std::map<size_t, uint32_t> &typeIndex2HeapConstantIndex)
{
    ...
    auto receiverHC = builder_.LoadConstOffset(VariableType::JS_POINTER(), info.tacc.GetReceiver(),
...
void TypedBytecodeLowering::LowerTypedPolyLdObjByName(const LoadObjByNameTypeInfoAccessor &tacc)
{
    DEFVALUE(result, (&builder_), VariableType::JS_ANY(), builder_.Hole());
