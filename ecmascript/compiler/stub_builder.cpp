                                    ...
                                    JSCallMode mode, std::initializer_list<GateRef> args)
{
    ...
    Bind(&methodIsNative);
    {
        ...
        if (AssemblerModule::JSModeIsCallThis(mode)) {
        ...
        switch (mode) {
            ...
            case JSCallMode::DEPRECATED_CALL_ARG1:
                result = CallNGCRuntime(glue, RTSTUB_ID(PushCallArgsAndDispatchNative),
                    { nativeCode, glue, numArgs, func, newTarget, thisValue, data[1]});
                break;
            case JSCallMode::CALL_THIS_ARG2:
            case JSCallMode::CALL_ARG2:
            case JSCallMode::DEPRECATED_CALL_ARG2:
                result = CallNGCRuntime(glue, RTSTUB_ID(PushCallArgsAndDispatchNative),
                    { nativeCode, glue, numArgs, func, newTarget, thisValue, data[1], data[2] });
                break;
            ...
            case JSCallMode::DEPRECATED_CALL_ARG3:
                result = CallNGCRuntime(glue, RTSTUB_ID(PushCallArgsAndDispatchNative),
                    { nativeCode, glue, numArgs, func,
                      newTarget, thisValue, data[1], data[2], data[3] }); // 2: args2
                break;
            ...
            case JSCallMode::DEPRECATED_CALL_THIS_WITH_ARGV: {
                ...
                Bind(&isFastBuiltins);
                {
                    ...
                    result = DispatchBuiltins(glue, builtinId, { glue, nativeCode, func, thisValue, data[1], data[2] });
                    Jump(&exit);
                }
                Bind(&notFastBuiltins);
                result = CallNGCRuntime(glue, RTSTUB_ID(PushCallRangeAndDispatchNative),
                    { glue, nativeCode, func, thisValue, data[1], data[2] });
                break;
            }
    ...
    Label methodNotAot(env);
    {
        ...
        Bind(&methodisAot);
        {
            ...
            switch (mode) {
                ...
                case JSCallMode::CALL_SETTER:
                    result = CallNGCRuntime(glue, RTSTUB_ID(JSCall),
                        { glue, lexEnv, realNumArgs, func, newTarget, thisValue, data[1] });
                    Jump(&exit);
                    break;
                case JSCallMode::CALL_THIS_ARG2:
                case JSCallMode::CALL_ARG2:
                case JSCallMode::DEPRECATED_CALL_ARG2:
                    result = CallNGCRuntime(glue, RTSTUB_ID(JSCall),
                        { glue, lexEnv, realNumArgs, func, newTarget, thisValue,  data[1], data[2] }); // 2: args2
                    Jump(&exit);
                    break;
                case JSCallMode::CALL_THIS_ARG3:
                case JSCallMode::CALL_ARG3:
                ...
                case JSCallMode::DEPRECATED_CALL_WITH_ARGV:
                    result = CallNGCRuntime(glue, RTSTUB_ID(JSCallWithArgV),
                        { glue, ZExtInt32ToInt64(actualNumArgs), func, newTarget, thisValue, data[2] }); // 2: args2
                    Jump(&exit);
                    break;
                case JSCallMode::DEPRECATED_CALL_CONSTRUCTOR_WITH_ARGV:
                case JSCallMode::CALL_CONSTRUCTOR_WITH_ARGV:
                    result = CallNGCRuntime(glue, RTSTUB_ID(ConstructorJSCallWithArgV),
                        { glue, ZExtInt32ToInt64(actualNumArgs), func, func, thisValue, data[2]});
                    result = ConstructorCheck(glue, func, *result, thisValue);  // 2: the second index
                    Jump(&exit);
                    break;
                default:
                    UNREACHABLE();
        ...
        switch (mode) {
            ...
            case JSCallMode::DEPRECATED_CALL_ARG0:
                result = CallNGCRuntime(glue, RTSTUB_ID(PushCallArg0AndDispatch),
                    { glue, sp, func, method, callField, thisValue });
                Return();
                break;
            case JSCallMode::CALL_THIS_ARG1:
            case JSCallMode::CALL_ARG1:
            case JSCallMode::DEPRECATED_CALL_ARG1:
                result = CallNGCRuntime(glue, RTSTUB_ID(PushCallArg1AndDispatch),
                    { glue, sp, func, method, callField, thisValue, data[1] });
                Return();
                break;
            case JSCallMode::CALL_THIS_ARG2:
            case JSCallMode::CALL_ARG2:
            case JSCallMode::DEPRECATED_CALL_ARG2:
                result = CallNGCRuntime(glue, RTSTUB_ID(PushCallArgs2AndDispatch),
                    { glue, sp, func, method, callField, thisValue, data[1], data[2] }); // 2: args2
                Return();
                break;
            case JSCallMode::CALL_THIS_ARG3:
            case JSCallMode::CALL_ARG3:
            case JSCallMode::DEPRECATED_CALL_ARG3:
                result = CallNGCRuntime(glue, RTSTUB_ID(PushCallArgs3AndDispatch),
                    { glue, sp, func, method, callField, thisValue, data[1], data[2], data[3] }); // 2: args2
                Return();
                break;
            case JSCallMode::CALL_WITH_ARGV:
            case JSCallMode::DEPRECATED_CALL_WITH_ARGV:
            ...
            case JSCallMode::DEPRECATED_CALL_THIS_WITH_ARGV:
                result = CallNGCRuntime(glue, RTSTUB_ID(PushCallThisRangeAndDispatch),
                    { glue, sp, func, method, callField, thisValue, data[1], data[2] });
                Return();
                break;
            case JSCallMode::DEPRECATED_CALL_CONSTRUCTOR_WITH_ARGV:
            case JSCallMode::CALL_CONSTRUCTOR_WITH_ARGV:
            ...
            case JSCallMode::CALL_GETTER:
                result = CallNGCRuntime(glue, RTSTUB_ID(CallGetter),
                    { glue, func, method, callField, thisValue });
                Jump(&exit);
                break;
            case JSCallMode::CALL_SETTER:
                result = CallNGCRuntime(glue, RTSTUB_ID(CallSetter),
                    { glue, func, method, callField, thisValue, data[1] });
                Jump(&exit);
                break;
            default:
                UNREACHABLE();
