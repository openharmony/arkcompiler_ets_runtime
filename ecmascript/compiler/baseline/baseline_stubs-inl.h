...
GateRef BaselineStubBuilder::GetModule(GateRef sp)
{
...
GateRef BaselineStubBuilder::GetThisFromFrame(GateRef frame)
{
    return Load(VariableType::JS_POINTER(), frame,
                IntPtr(AsmInterpretedFrame::GetThisOffset(GetEnvironment()->IsArch32Bit())));
}

GateRef BaselineStubBuilder::GetNewTarget(GateRef sp)
{
...
GateRef BaselineStubBuilder::GetStartIdxAndNumArgs(GateRef sp, GateRef restIdx)
{
    auto env = GetEnvironment();
    Label subEntry(env);
...
GateRef BaselineStubBuilder::GetVregValue(GateRef sp, GateRef idx)
{
