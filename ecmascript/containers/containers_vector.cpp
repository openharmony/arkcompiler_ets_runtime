...
JSTaggedValue ContainersVector::GetCapacity(EcmaRuntimeCallInfo *argv)
{
    ...
    uint32_t capacity = JSHandle<JSAPIVector>::Cast(self)->GetCapacity(thread);
    RETURN_EXCEPTION_IF_ABRUPT_COMPLETION(thread);
...
JSTaggedValue ContainersVector::GetLastElement(EcmaRuntimeCallInfo *argv)
{
    ...
    JSTaggedValue value = JSHandle<JSAPIVector>::Cast(self)->GetLastElement(thread);
...
JSTaggedValue ContainersVector::ForEach(EcmaRuntimeCallInfo *argv)
{
    ...
    if (!thisHandle->IsJSAPIVector()) {
        if (thisHandle->IsJSProxy() && JSHandle<JSProxy>::Cast(thisHandle)->GetTarget(thread).IsJSAPIVector()) {
...
JSTaggedValue ContainersVector::Has(EcmaRuntimeCallInfo *argv)
{
    ...
    JSHandle<JSTaggedValue> value(GetCallArg(argv, 0));
    bool isHas = JSHandle<JSAPIVector>::Cast(self)->Has(thread, value.GetTaggedValue());
...
JSTaggedValue ContainersVector::ConvertToArray(EcmaRuntimeCallInfo *argv)
{
    ...
    JSHandle<TaggedArray> srcElements(thread, vector->GetElements(thread));
...
JSTaggedValue ContainersVector::GetFirstElement(EcmaRuntimeCallInfo *argv)
{
    ...
    JSTaggedValue firstElement = JSAPIVector::GetFirstElement(thread, JSHandle<JSAPIVector>::Cast(self));
    return firstElement;
}
...
JSTaggedValue ContainersVector::Sort(EcmaRuntimeCallInfo *argv)
{
    ...
    JSHandle<JSAPIVector> vector = JSHandle<JSAPIVector>::Cast(self);
    uint32_t length = static_cast<uint32_t>(vector->GetSize());
    ...
    JSHandle<TaggedArray> dstElements(thread, vector->GetElements(thread));
    if (dstElements->GetLength() < length) {
        ...
        vector->SetElements(thread, sortedArray);
