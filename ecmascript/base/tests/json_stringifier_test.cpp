...
/**
 * @tc.name: Stringify_009
 * @tc.desc: Check whether the result returned through "Stringify" function is within expectations
 *           the first parameter of the ECMAObject,the second parameter is JSSharedArray,the third parameter
 *           is String.This situation will stringize parameters through "SerializeJSONObject" function.
 * @tc.type: FUNC
 * @tc.require:
 */
HWTEST_F_L0(JsonStringifierTest, Stringify_009)
{
    ...
    JSSharedArray *handleArr =
        JSSharedArray::Cast(JSSharedArray::ArrayCreate(thread, JSTaggedNumber(0)).GetTaggedValue().GetTaggedObject());
    JSHandle<JSObject> handleObj(thread, handleArr);

    JSHandle<JSTaggedValue> handleKey0(thread, JSTaggedValue(0));
    PropertyDescriptor handleValue0(thread, JSHandle<JSTaggedValue>(thread, JSTaggedValue(*factory->NewFromASCII("json"))), true, true, true);
    JSSharedArray::DefineOwnProperty(thread, handleObj, handleKey0, handleValue0);

    JSHandle<JSTaggedValue> handleKey1(thread, JSTaggedValue(1));
    PropertyDescriptor handleDesc(thread, JSHandle<JSTaggedValue>(thread, JSTaggedValue(100)), true, true, true);
    JSSharedArray::DefineOwnProperty(thread, handleObj, handleKey1, handleDesc);

    JSHandle<JSTaggedValue> handleKey2(thread, JSTaggedValue(2));
    PropertyDescriptor handleValue2(thread, JSHandle<JSTaggedValue>(thread,
        JSTaggedValue(*factory->NewFromASCII("abc"))), true, true, true);
    JSSharedArray::DefineOwnProperty(thread, handleObj, handleKey2, handleValue2);
...
HWTEST_F_L0(JsonStringifierTest, Stringify_010)
{
    ...
    JSHandle<JSTaggedValue> handleKey0(thread, JSTaggedValue(0));
    PropertyDescriptor handleDesc0(thread, JSHandle<JSTaggedValue>(thread, JSTaggedValue(1)), true, true, true);
    JSSharedArray::DefineOwnProperty(thread, handleObj, handleKey0, handleDesc0);

    JSHandle<JSTaggedValue> handleKey1(thread, JSTaggedValue(1));
    PropertyDescriptor handleDesc1(thread, JSHandle<JSTaggedValue>(thread, JSTaggedValue(3.6)), true, true, true);
    JSSharedArray::DefineOwnProperty(thread, handleObj, handleKey1, handleDesc1);
