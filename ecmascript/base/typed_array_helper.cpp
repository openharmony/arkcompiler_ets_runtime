                                                           ...
                                                           const DataViewType arrayType)
{
    ...
    if (arrayType == srcType && srcArray->IsSharedTypedArray()) {
        JSTaggedValue tmp = BuiltinsSendableArrayBuffer::CloneArrayBuffer(
            thread, srcData, srcByteOffset, globalConst->GetHandledUndefined());
        data.Update(tmp);
        RETURN_EXCEPTION_IF_ABRUPT_COMPLETION(thread);
    } else {
        // 19. Else,
        ...
        JSTaggedValue tmp = BuiltinsSendableArrayBuffer::AllocateSendableArrayBuffer(
            thread, bufferConstructor, byteLength);
        RETURN_EXCEPTION_IF_ABRUPT_COMPLETION(thread);
        data.Update(tmp);
        RETURN_EXCEPTION_IF_ABRUPT_COMPLETION(thread);
        //   b. If IsDetachedBuffer(srcData) is true, throw a TypeError exception.
...
JSTaggedValue TypedArrayHelper::findCommon(EcmaRuntimeCallInfo *argv,
...
JSTaggedValue TypedArrayHelper::findIndexCommon(EcmaRuntimeCallInfo *argv,
                                                JSHandle<JSTaggedValue> thisObjVal, uint64_t len)
{
    JSThread *thread = argv->GetThread();
    // 5. If IsCallable(predicate) is false, throw a TypeError exception.
    JSHandle<JSTaggedValue> callbackFnHandle = BuiltinsBase::GetCallArg(argv, 0);
    ...
    //   g. Increase k by 1.
    uint32_t k = 0;
    JSTaggedValue callResult = BuiltinsBase::GetTaggedBoolean(true);
    if (thisObjVal->IsStableJSArray(thread)) {
        callResult = JSStableArray::HandleFindIndexOfStable(thread, JSHandle<JSObject>::Cast(thisObjVal),
                                                            callbackFnHandle, thisArgHandle, k);
        RETURN_EXCEPTION_IF_ABRUPT_COMPLETION(thread);
        if (callResult.ToBoolean()) {
            return BuiltinsBase::GetTaggedDouble(k);
        }
    }
    JSMutableHandle<JSTaggedValue> key(thread, JSTaggedValue::Undefined());
    JSHandle<JSTaggedValue> undefined = thread->GlobalConstants()->GetHandledUndefined();
...
JSTaggedValue TypedArrayHelper::findLastCommon(EcmaRuntimeCallInfo *argv,
                                               JSHandle<JSTaggedValue> thisObjVal, int64_t len)
{
    JSThread *thread = argv->GetThread();
    // 5. If IsCallable(predicate) is false, throw a TypeError exception.
    ...
    if (thisObjVal->IsStableJSArray(thread)) {
        ...
        callResult = JSStableArray::HandleFindLastOfStable(thread, JSHandle<JSObject>::Cast(thisObjVal),
...
JSTaggedValue TypedArrayHelper::findLastIndexCommon(EcmaRuntimeCallInfo *argv,
                                                    JSHandle<JSTaggedValue> thisObjVal, int64_t len)
{
    JSThread *thread = argv->GetThread();
    // 5. If IsCallable(predicate) is false, throw a TypeError exception.
    ...
    if (thisObjVal->IsStableJSArray(thread)) {
        callResult =
            JSStableArray::HandleFindLastIndexOfStable(thread, JSHandle<JSObject>::Cast(thisObjVal),
                                                       callbackFnHandle, thisArgHandle, k);
            RETURN_EXCEPTION_IF_ABRUPT_COMPLETION(thread);
        if (callResult.ToBoolean()) {
            return BuiltinsBase::GetTaggedDouble(k);
