...
HWTEST_F_L0(ICRunTimeTest, UpdateLoadHandler)
{
    ...
    for (uint32_t i = 0; i < arrayLength; i++) {
        ...
        handleProfileTypeInfo->SetICSlot(thread, i, undefinedVal.GetTaggedValue());
    }
    // test op is Element
...
HWTEST_F_L0(ICRunTimeTest, UpdateStoreHandler)
{
    ...
    EXPECT_TRUE(handleProfileTypeInfo->GetICSlot(thread, slotId + 1).IsPrototypeHandler());
    ...
    EXPECT_TRUE(handleProfileTypeInfo->GetICSlot(thread, slotId).IsWeak());
    EXPECT_TRUE(handleProfileTypeInfo->GetICSlot(thread, slotId + 1).IsTransitionHandler());
}
...
HWTEST_F_L0(ICRunTimeTest, StoreMiss)
{
    ...
    JSHandle<ProfileTypeInfo> handleProfileTypeInfo = factory->NewProfileTypeInfo(arrayLength);
    handleProfileTypeInfo->SetICSlot(thread, 0, JSTaggedValue::Hole());
    ...
    handleProfileTypeInfo->SetICSlot(thread, 0, JSTaggedValue::Undefined());
    storeICRuntime.StoreMiss(handleReceiver1, handleKeyWithString, handleValueWithElement);
    EXPECT_TRUE(handleProfileTypeInfo->GetICSlot(thread, 0).IsPropertyBox());
}
