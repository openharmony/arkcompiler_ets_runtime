...
public:
    ...
    ACCESSORS(ImportName, IMPORT_ENTRY_OFFSET, LOCAL_NAME_OFFSET);
    ACCESSORS(LocalName, LOCAL_NAME_OFFSET, MODULE_REQUEST_INDEX_OFFSET);
    ACCESSORS_PRIMITIVE_FIELD(ModuleRequestIndex, uint32_t, MODULE_REQUEST_INDEX_OFFSET, LAST_OFFSET);
    DEFINE_ALIGN_SIZE(LAST_OFFSET);

    DECL_DUMP()
    DECL_VISIT_OBJECT(IMPORT_ENTRY_OFFSET, MODULE_REQUEST_INDEX_OFFSET)
};
...
public:
    ...
    ACCESSORS(ExportName, INDIRECT_EXPORT_ENTRY_OFFSET, IMPORT_NAME_OFFSET);
    ACCESSORS(ImportName, IMPORT_NAME_OFFSET, MODULE_REQUEST_INDEX_OFFSET);
    ACCESSORS_PRIMITIVE_FIELD(ModuleRequestIndex, uint32_t, MODULE_REQUEST_INDEX_OFFSET, LAST_OFFSET);
    DEFINE_ALIGN_SIZE(LAST_OFFSET);

    DECL_DUMP()
    DECL_VISIT_OBJECT(INDIRECT_EXPORT_ENTRY_OFFSET, MODULE_REQUEST_INDEX_OFFSET)
};
...
public:
    ...
    ACCESSORS_PRIMITIVE_FIELD(ModuleRequestIndex, uint32_t, STAR_EXPORT_ENTRY_OFFSET, LAST_OFFSET);
    DEFINE_ALIGN_SIZE(LAST_OFFSET);

    DECL_DUMP()
    DECL_VISIT_OBJECT(STAR_EXPORT_ENTRY_OFFSET, STAR_EXPORT_ENTRY_OFFSET)
};
}  // namespace panda::ecmascript
