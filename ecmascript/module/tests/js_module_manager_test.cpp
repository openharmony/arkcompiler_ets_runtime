...
HWTEST_F_L0(ModuleManagerTest, ModuleManager_TryGetPendingRemovalModule_NotFound)
{
    ModuleManager *moduleManager = thread->GetModuleManager();

    EXPECT_TRUE(moduleManager->TryGetPendingRemovalModule("not_in_pending_buffer")->IsUndefined());
}

HWTEST_F_L0(ModuleManagerTest, ModuleManager_ProcessPendingRemovalLocalModules_DeadModule)
{
    ModuleManager *moduleManager = thread->GetModuleManager();
    CString recordName = "pending_dead_module";
    JSHandle<SourceTextModule> module = instance->GetFactory()->NewSourceTextModule();
    module->SetEcmaModuleFilenameString(thread, "pending_dead_module.abc");
    ...
    WeakRootVisitor visitor = [](TaggedObject *) -> TaggedObject * {
        return nullptr;
    };
    ...
    EXPECT_EQ(module->GetEcmaModuleRecordName(), 0U);
}

HWTEST_F_L0(ModuleManagerTest, ModuleManager_ProcessPendingRemovalLocalModules_ForwardedModule)
{
    ModuleManager *moduleManager = thread->GetModuleManager();
    CString recordName = "pending_forwarded_module";
    JSHandle<SourceTextModule> module = instance->GetFactory()->NewSourceTextModule();
    JSHandle<SourceTextModule> forwardedModule = instance->GetFactory()->NewSourceTextModule();
    module->SetEcmaModuleRecordNameString(recordName);
    module->SetLoadingTypes(LoadingTypes::DYNAMITC_MODULE);
    forwardedModule->SetEcmaModuleRecordNameString(recordName);
    forwardedModule->SetLoadingTypes(LoadingTypes::DYNAMITC_MODULE);
    moduleManager->AddResolveImportedModule(recordName, module.GetTaggedValue());
    ModuleDeregister::RemoveModule(thread, module);
    WeakRootVisitor visitor = [forwarded = forwardedModule.GetTaggedValue()](TaggedObject *) -> TaggedObject * {
        return forwarded.GetTaggedObject();
    };

    instance->ProcessPendingRemovalLocalModules(visitor);

    JSHandle<JSTaggedValue> restoredModule = moduleManager->TryGetPendingRemovalModule(recordName);
    EXPECT_EQ(restoredModule.GetTaggedValue(), forwardedModule.GetTaggedValue());
    EXPECT_TRUE(moduleManager->IsLocalModuleLoaded(recordName));
    module->DestroyModuleCNativeFields(moduleManager);
}

HWTEST_F_L0(ModuleManagerTest, EcmaVM_ProcessPendingRemovalLocalModules_NoJSThread)
{
    bool visitorCalled = false;
    WeakRootVisitor visitor = [&visitorCalled](TaggedObject *object) -> TaggedObject * {
        visitorCalled = true;
        return object;
    };
    instance->SetJSThread(nullptr);

    instance->ProcessPendingRemovalLocalModules(visitor);

    instance->SetJSThread(thread);
    EXPECT_FALSE(visitorCalled);
}

HWTEST_F_L0(ModuleManagerTest, EcmaVM_ProcessPendingRemovalLocalModules_NoModuleManager)
{
    JSHandle<GlobalEnv> globalEnv = thread->GetGlobalEnv();
    JSTaggedValue moduleManagerNativePointer = globalEnv->GetModuleManagerNativePointer().GetTaggedValue();
    globalEnv->SetModuleManagerNativePointer(thread, JSTaggedValue::Undefined());
    bool visitorCalled = false;
    WeakRootVisitor visitor = [&visitorCalled](TaggedObject *object) -> TaggedObject * {
        visitorCalled = true;
        return object;
    ...
    globalEnv->SetModuleManagerNativePointer(thread, moduleManagerNativePointer);
    EXPECT_FALSE(visitorCalled);
}
