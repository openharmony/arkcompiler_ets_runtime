...
std::string EcmaStringAccessor::ToStdString(StringConvertedUsage usage)
{
    ...
    CVector<uint8_t> buf;
    common::Span<const uint8_t> sp = string_->ToUtf8Span(buf, modify);
#if ENABLE_NEXT_OPTIMIZATION
...
std::string EcmaStringAccessor::DebuggerToStdString(StringConvertedUsage usage)
{
    ...
    bool modify = (usage != StringConvertedUsage::PRINT);
    CVector<uint8_t> buf;
    common::Span<const uint8_t> sp = string_->DebuggerToUtf8Span(buf, modify);
#if ENABLE_NEXT_OPTIMIZATION
    return std::string(reinterpret_cast<const char*>(sp.data()), sp.size());
#else
    std::string res;
...
CString EcmaStringAccessor::ToCString(StringConvertedUsage usage, bool cesu8)
{
    ...
    bool modify = (usage != StringConvertedUsage::PRINT);
    CVector<uint8_t> buf;
    common::Span<const uint8_t> sp = string_->ToUtf8Span(buf, modify, cesu8);
#if ENABLE_NEXT_OPTIMIZATION
...
void EcmaStringAccessor::AppendToCString(CString &str)
{
    ...
    const uint8_t *data = LineString::GetUtf8DataFlat(std::move(readBarrier), BaseString::ConstCast(string_), buf);
...
void EcmaStringAccessor::AppendToC16String(C16String &str)
{
    ...
    if LIKELY(string_->IsUtf8()) {
        ...
        const uint16_t* data =
            LineString::GetUtf16DataFlat(std::move(readBarrier), BaseString::ConstCast(string_), buf);
