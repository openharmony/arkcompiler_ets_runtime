...
void SweepGC::Sweep()
{
    ...
    ProcessNativeDelete();
    size_t totalSurvivalSize = 0;
    heap_->GetSlotSpace()->EnumerateRegions([&totalSurvivalSize](Region *region) {
        region->SetGCAliveSize();
        totalSurvivalSize += region->GetGCAliveSize();
    });
    heap_->GetSlotSpace()->SetSurvivalObjectSize(totalSurvivalSize);
