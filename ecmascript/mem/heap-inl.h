...
{
#ifdef USE_CMC_GC
    return reinterpret_cast<TaggedObject *>(HeapAllocator::Allocate(size, Language::DYNAMIC));
#else
...
#ifdef USE_CMC_GC
    // check sync
    return HeapAllocator::Allocate(size, Language::DYNAMIC);
#else
...
#ifdef USE_CMC_GC
    auto object = reinterpret_cast<TaggedObject*>(HeapAllocator::Allocate(size, Language::DYNAMIC));
#else
...
#ifdef USE_CMC_GC
    if (size > MAX_REGULAR_HEAP_OBJECT_SIZE) {
        object = reinterpret_cast<TaggedObject *>(HeapAllocator::AllocateInHuge(size, Language::DYNAMIC));
    } else {
        object = reinterpret_cast<TaggedObject *>(HeapAllocator::AllocateInOld(size, Language::DYNAMIC));
    }
#else
    if (size > MAX_REGULAR_HEAP_OBJECT_SIZE) {
        object = AllocateHugeObject(size);
...
{
#ifdef USE_CMC_GC
    return reinterpret_cast<TaggedObject *>(HeapAllocator::AllocateInReadOnly(size, Language::DYNAMIC));
#else
...
#ifdef USE_CMC_GC
        object = reinterpret_cast<TaggedObject *>(HeapAllocator::AllocateInNonmove(size, Language::DYNAMIC));
#else
...
{
#ifdef USE_CMC_GC
    return reinterpret_cast<TaggedObject *>(HeapAllocator::AllocateInHuge(size, Language::DYNAMIC));
#else
...
#ifdef USE_CMC_GC
            reinterpret_cast<TaggedObject *>(HeapAllocator::AllocateInHuge(size, Language::DYNAMIC)) :
            reinterpret_cast<TaggedObject *>(HeapAllocator::AllocateInNonmove(size, Language::DYNAMIC));
#else
...
{
#ifdef USE_CMC_GC
    return reinterpret_cast<TaggedObject*>(HeapAllocator::AllocateInNonmove(size, Language::DYNAMIC));
#endif
...
TaggedObject *Heap::AllocateSharedOldSpaceFromTlab(JSThread *thread, size_t size)
{
    ...
    return reinterpret_cast<TaggedObject*>(HeapAllocator::AllocateInOld(size, Language::DYNAMIC));
#endif
...
#ifdef USE_CMC_GC
    TaggedObject *object = reinterpret_cast<TaggedObject*>(HeapAllocator::AllocateInOld(size, Language::DYNAMIC));
    object->SetClass(thread, hclass);
#else
    TaggedObject *object = thread->IsJitThread() ? nullptr :
...
#ifdef USE_CMC_GC
    TaggedObject *object = thread->IsJitThread() ? nullptr :
                           reinterpret_cast<TaggedObject*>(HeapAllocator::AllocateInOld(size, Language::DYNAMIC));
#else
    TaggedObject *object = thread->IsJitThread() ? nullptr :
        const_cast<Heap*>(thread->GetEcmaVM()->GetHeap())->AllocateSharedOldSpaceFromTlab(thread, size);
    if (object == nullptr) {
