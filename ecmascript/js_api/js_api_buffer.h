...
public:
    enum EncodingType { ASCII = 1, UTF8, UTF16LE, BASE64, BASE64URL, LATIN1, BINARY, HEX };
    enum NumberSize {
        UINT8 = 1,
        INT8 = 1,
        UINT16 = 2,
    ...
    static JSTaggedValue ReadBytes(JSThread *thread, JSHandle<JSAPIFastBuffer> &buffer, int32_t offset,
                                   ByteLength byteLength, bool littleEndian = false);
    static JSTaggedValue ReadInt(JSThread *thread, JSHandle<JSAPIFastBuffer> &buffer, int32_t offset,
                                 ByteLength byteLength, bool littleEndian = false);
    ...
    static std::string GetString(std::string &value, EncodingType encodingType);
    ...
    static JSTaggedValue AllocateEmptyData(JSThread *thread, const JSHandle<JSAPIFastBuffer> &buffer,
                                           uint32_t byteLength = 0, uint32_t byteOffset = 0);
    ...
    static EncodingType GetEncodingType(std::string_view encode);
    ...
    ACCESSORS_PRIMITIVE_FIELD(Length, uint32_t, BUFFER_LENGTH_OFFSET, LAST_OFFSET)

    inline void SetOffset(uint32_t offset)
    {
        JSTypedArray::Cast(this->GetFastBufferData().GetTaggedObject())->SetByteOffset(offset);
    }

    inline uint32_t GetOffset() const
    {
        return JSTypedArray::Cast(this->GetFastBufferData().GetTaggedObject())->GetByteOffset();
    }
...
private:
    ...
    static bool WriteDataLoop(JSThread *thread, const uint8_t *src, uint8_t *dest, uint32_t length, uint32_t start,
                              uint32_t end);
    ...
    static int32_t StringMatch(JSThread *thread, JSHandle<JSAPIFastBuffer> &buffer, const uint8_t *str,
