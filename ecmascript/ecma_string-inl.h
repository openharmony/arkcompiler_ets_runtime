                                              ...
                                              bool canBeCompress, MemSpaceType type)
{
    ...
    auto allocator = [vm, type](size_t size, ObjectType stringType) -> BaseObject* {
        ASSERT(stringType == ObjectType::LINE_STRING && "Can only allocate line string");
...
{
    auto allocator = [vm](size_t size, ObjectType stringType) -> BaseObject* {
        ASSERT(stringType == ObjectType::LINE_STRING && "Can only allocate line string");
...
inline SlicedEcmaString* EcmaString::CreateSlicedString(const EcmaVM* vm, JSHandle<EcmaString> parent,
                                                        MemSpaceType type)
{
    auto allocator = [vm, type](size_t, ObjectType stringType) -> BaseObject* {
        ASSERT(stringType == ObjectType::SLICED_STRING && "Can only allocate sliced string");
    ...
    const JSHandle<EcmaString> &left, const JSHandle<EcmaString> &right, uint32_t length, bool compressed)
{
    ...
    auto allocator = [vm](size_t, ObjectType stringType) -> BaseObject* {
        ASSERT(stringType == ObjectType::TREE_STRING && "Can only allocate tree string");
