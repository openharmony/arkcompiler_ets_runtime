...
function Operation()
{
    ...
    // The following expression triggers spread call enters a runtime call that executes callable.
    // That runtime-stub bytecode site is where the compiled run triggers lazy-deopt.
    let result = callable(...callArgs);
    let after = obj.x;
    let tail = 0;
    for (let i = 0; i < 3; i++) {
        tail += after + i;
...
runCase("lazy_deopt_runtime_stub_callspread_1");
