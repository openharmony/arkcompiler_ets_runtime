...
function Operation()
{
    ...
    // The following expression triggers range construction enters a runtime constructor path.
    // That runtime-stub bytecode site is where the compiled run triggers lazy-deopt.
    let result = (new ConstructTarget(2, 4)).value;
    let after = obj.x;
    let tail = 0;
    for (let i = 0; i < 3; i++) {
        tail += after + i;
...
runCase("lazy_deopt_runtime_stub_newobjrange_1");
