...
function Operation()
{
    ...
    // Dependency invalidation immediately after it checks that the compiled continuation
    // deopts and resumes with the restored frame state.
    let created = { __proto__: prototypeObject, value: 8 };
    invalidateDependency();
    let result = created.value;
    let after = obj.x;
    let tail = 0;
    for (let i = 0; i < 3; i++) {
        tail += after + i;
...
runCase("lazy_deopt_runtime_stub_setobjectwithproto_1");
