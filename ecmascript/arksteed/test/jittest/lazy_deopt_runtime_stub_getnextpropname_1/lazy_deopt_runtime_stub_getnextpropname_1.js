...
const enumObject = { alpha: 1, beta: 2, gamma: 3 };
...
function Operation()
{
    ...
    for (let key in enumObject) {
        // GETNEXTPROPNAME advances the for-in iterator. The first iteration then mutates
        // the prototype dependency so the compiled continuation must lazy-deopt here.
        result += key;
        if (index === 0) {
...
runCase("lazy_deopt_runtime_stub_getnextpropname_1");
