...
class SubjectCtor extends BaseCtor {
    constructor(value) {
        let before = obj.x;
        let marker = before + 10;
        let callSuper = () => {
            // SUPERCALLARROWRANGE is the selected runtime-stub bytecode. The arrow call
            // performs the constructor's first super(), so it is a legal initialization path.
            return super(value);
        };
        let nextThis = callSuper();
        let after = obj.x;
        nextThis.state = before + ":" + nextThis.baseValue + ":" + after + ":" + marker;
        return nextThis;
    }
}
...
function runCase(name)
{
    ...
    Test(name);
    printCompiledState("After 2nd call", true);
    print("----------------");
}

runCase("lazy_deopt_runtime_stub_supercallarrowrange_1");
