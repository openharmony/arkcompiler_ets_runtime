...
const sourceObject = new Proxy({k: 5, 0: 6}, {
...
function Operation()
{
    ...
    // The following expression triggers object spread copies properties from a Proxy source.
    // That runtime-stub bytecode site is where the compiled run triggers lazy-deopt.
    let result = ({seed: 1, ...sourceObject}).k;
    let after = obj.x;
    let tail = 0;
    for (let i = 0; i < 3; i++) {
        tail += after + i;
...
runCase("lazy_deopt_runtime_stub_copydataproperties_1");
