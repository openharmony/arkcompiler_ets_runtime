...
function Operation()
{
    ...
    // The following expression triggers numeric conversion in exponentiation calls triggerValue.valueOf()/toString().
    // That runtime-stub bytecode site is where the compiled run triggers lazy-deopt.
    let result = triggerValue ** 2;
    let after = obj.x;
    let tail = 0;
    for (let i = 0; i < 3; i++) {
        tail += after + i;
...
runCase("lazy_deopt_runtime_stub_exp_1");
