...
function Operation()
{
    ...
    // The following expression triggers super keyed store invokes the BaseForSuper setter.
    // That runtime-stub bytecode site is where the compiled run triggers lazy-deopt.
    let result = superReceiver.stByValue("k", 12);
    let after = obj.x;
    let tail = 0;
    for (let i = 0; i < 3; i++) {
        tail += after + i;
...
runCase("lazy_deopt_runtime_stub_stsuperbyvalue_1");
