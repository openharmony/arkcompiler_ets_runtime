...
#ifndef ECMASCRIPT_STRING_TABLE_HASHTRIEMAP_H
#define ECMASCRIPT_STRING_TABLE_HASHTRIEMAP_H
...
public:
    HashTrieMapEntry(BaseString* v) : overflow_(nullptr)
    {
        ...
        ASSERT_LOGF(common::Heap::GetHeap().IsHeapAddress(v)
                        ? common::Heap::GetHeap().InRecentSpace(v) == false
                        : true,
...
public:
    ...
    BaseString* StoreOrLoad(ThreadHolder* holder, ReadBarrier&& readBarrier, const uint32_t key, LoadResult loadResult,
                            HandleType str);
    template <typename ReadBarrier>
    LoadResult Load(ReadBarrier&& readBarrier, const uint32_t key, const common::ReadOnlyHandle<BaseString>& string,
