...
public:
    ...
    void Init(uint32_t pageCount)
    {
        ...
        smallPageUsed_ = 0;
        usedZone_ = 0;
        size_t size = static_cast<size_t>(totalPageCount_) * panda::ARK_COMMON_PAGE_SIZE;
...
#else
        LOGE_IF(munmap(base_, totalPageCount_ * panda::ARK_COMMON_PAGE_SIZE) != EOK) <<
                "munmap failed in PagePool destruction, errno: " << errno;
#endif
    }

    uint8_t* GetPage(size_t bytes = panda::ARK_COMMON_PAGE_SIZE)
    {
        uint32_t idx = 0;
        size_t count = (bytes + panda::ARK_COMMON_PAGE_SIZE - 1) / panda::ARK_COMMON_PAGE_SIZE;
        size_t pageSize = RoundUp(bytes, panda::ARK_COMMON_PAGE_SIZE);
        LOGF_CHECK(count < std::numeric_limits<uint32_t>::max()) << "native memory out of memory!";
        {
            ...
            if (freePagesTree_.TakeUnits(static_cast<uint32_t>(count), idx, false)) {
                auto* ret = base_ + static_cast<size_t>(idx) * panda::ARK_COMMON_PAGE_SIZE;
#ifdef _WIN64
    ...
    void ReturnPage(uint8_t* page, size_t bytes = panda::ARK_COMMON_PAGE_SIZE) noexcept
    {
        uint8_t* end = base_ + totalSize_;
        size_t num = (bytes + panda::ARK_COMMON_PAGE_SIZE - 1) / panda::ARK_COMMON_PAGE_SIZE;
...
#else
            LOGE_IF(UNLIKELY_CC(munmap(page, num * panda::ARK_COMMON_PAGE_SIZE) != EOK)) <<
                "munmap failed in ReturnPage, errno: " << errno;
#endif
            return;
        }
        LOGF_CHECK(num < std::numeric_limits<uint32_t>::max()) << "native memory out of memory!";
        uint32_t idx = static_cast<uint32_t>((page - base_) / panda::ARK_COMMON_PAGE_SIZE);
#if defined(_WIN64)
        LOGE_IF(UNLIKELY_CC(!VirtualFree(page, num * panda::ARK_COMMON_PAGE_SIZE, MEM_DECOMMIT))) <<
            "VirtualFree failed in ReturnPage, errno: " << GetLastError();
#elif defined(__APPLE__)
        panda::MemorySet(reinterpret_cast<uintptr_t>(page), num * panda::ARK_COMMON_PAGE_SIZE, 0,
                         num * panda::ARK_COMMON_PAGE_SIZE);
        (void)madvise(page, num * panda::ARK_COMMON_PAGE_SIZE, MADV_DONTNEED);
#else
        (void)madvise(page, num * panda::ARK_COMMON_PAGE_SIZE, MADV_DONTNEED);
#endif
        std::lock_guard<std::mutex> lg(freePagesMutex_);
...
#if defined(__linux__) || defined(PANDA_TARGET_OHOS)
        ARK_COMMON_PRCTL(result, size, memName);
#endif
