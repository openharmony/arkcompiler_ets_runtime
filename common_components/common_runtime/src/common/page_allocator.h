 ...
 */
#ifndef ARK_COMMON_PAGEALLOCATOR_H
#define ARK_COMMON_PAGEALLOCATOR_H
...
public:
    ...
    {
        slotAlignment_ = panda::AlignUp<uint16_t>(size, AllocatorUtils::ALLOC_ALIGNMENT);
    }
    ...
    void Deallocate(void* slot)
    {
        Page* current = reinterpret_cast<Page*>(
            panda::AlignDown<uintptr_t>(reinterpret_cast<uintptr_t>(slot), AllocatorUtils::ALLOC_PAGE_SIZE));
...
private:
    ...
    void InitPage(Page& page)
    {
        page.prev_ = nullptr;
        page.next_ = nullptr;
        constexpr uint32_t offset = panda::AlignUp<uint32_t>(sizeof(Page), AllocatorUtils::ALLOC_ALIGNMENT);
...
public:
    ...
    {
        uint32_t alignedSize = panda::AlignUp(static_cast<uint32_t>(size), AllocatorUtils::ALLOC_ALIGNMENT);
