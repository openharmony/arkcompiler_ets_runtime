...
namespace panda {
Address HeapAllocator::Allocate(size_t size, Language language)
{
...
Address HeapAllocator::AllocateInNonmove(size_t size, Language language)
{
...
Address HeapAllocator::AllocateInOld(size_t size, Language language)
{
    auto address = HeapManager::Allocate(size);
    BaseObject::Cast(address)->SetLanguage(language);
    return address;
...
Address HeapAllocator::AllocateInHuge(size_t size, Language language)
{
    auto address = HeapManager::Allocate(size);
    BaseObject::Cast(address)->SetLanguage(language);
    return address;
...
uintptr_t HeapAllocator::AllocateLargeJitFortRegion(size_t size, Language language)
{
