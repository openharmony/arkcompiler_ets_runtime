...
function testChildCornerCases() {
    ...
    runChildCornerCase(ChildNull, null);                // error: super is not JSFunction
    runChildCornerCase(ChildObj, {xxx: "yyy"});         // error: super is not JSFunction
    runChildCornerCase(ChildNumber, 111);               // normal
    runChildCornerCase(ChildString, "hello");           // normal
    runChildCornerCase(ChildArrow, () => {});           // error: constructor is false
