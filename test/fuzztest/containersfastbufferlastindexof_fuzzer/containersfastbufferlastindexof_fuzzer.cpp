...
#include "containersfastbufferlastindexof_fuzzer.h"
...
namespace OHOS {
    ...
    void ContainersFastBufferLastIndexOfFuzzTest(const uint8_t* data, size_t size)
    {
        ...
        EcmaVM *vm = JSNApi::CreateJSVM(option);
        {
            ...
            int32_t byteoffset = fdp.ConsumeIntegral<int32_t>();
            ...
            if (!rdStr.empty() && !encoding.empty()) {
                ...
                callInfo2->SetCallArg(1, JSTaggedValue(byteoffset));
                // 2 : means the third parameter
                callInfo2->SetCallArg(2, JSTaggedValue(*encodingType));
                ret = ContainersBuffer::LastIndexOf(callInfo2);
            }
        }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::ContainersFastBufferLastIndexOfFuzzTest(data, size);
    return 0;
}