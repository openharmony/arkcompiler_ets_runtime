...
let a1: { new <T>(x: T): T };
AssertType(a1, "new <T>(T) => T");
AssertType(x, "T");

let b1: { new (x: string): string };
AssertType(b1, "new (string) => string");
AssertType(x, "string");

let a2: { new <T>(x: T): T };
AssertType(a2, "new <T>(T) => T");
AssertType(x, "T");

let b2: { new (x: string, y: number): string };
AssertType(b2, "new (string, number) => string");
AssertType(x, "string");
AssertType(y, "number");

let a3: { new <T, U>(x: T, y: U): T };
AssertType(a3, "new <T, U>(T, U) => T");
AssertType(x, "T");
AssertType(y, "U");

let b3: { new (x: string, y: number): string };
AssertType(b3, "new (string, number) => string");
AssertType(x, "string");
AssertType(y, "number");

let a4: { new <T>(x?: T): T };
AssertType(a4, "new <T>(?T) => T");
AssertType(x, "T");

let b4: { new (x?: string): string };
AssertType(b4, "new (?string) => string");
AssertType(x, "string");

let a5: { new <T>(...x: T[]): T };
AssertType(a5, "new <T>(...T[]) => T");
AssertType(x, "T[]");

let b5: { new (...x: string[]): string };
AssertType(b5, "new (...string[]) => string");
AssertType(x, "string[]");

let a6: { new <T>(x: T, y: T): T };
AssertType(a6, "new <T>(T, T) => T");
AssertType(x, "T");
AssertType(y, "T");

let b6: { new (x: string, y: number): {} };
